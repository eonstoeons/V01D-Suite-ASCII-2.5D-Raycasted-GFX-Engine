...
class VehicleRenderer:
    ...
    def render(self, player, router):
        ...
        # Show yaw indicator in HUD when looking sideways
        yaw_deg = int(math.degrees(cam.cam_yaw))
        yaw_str = f"  CAM:{yaw_deg:+d}\u00b0" if abs(yaw_deg) > 2 else ""
        _buf_write(buf,2,H-2,f"[{biome}] GEAR:{cam.gear} RPM:{int(cam.rpm)} SPD:{cam.speed:+.1f}  VEHICLE MODE{yaw_str}",W,H)
        _buf_write(buf,2,H-1,"W/S throttle  A/D steer  \u25c4\u25ba look left/right  SHIFT boost  E exit  F takeoff",W,H)
        return "\n".join("".join(row) for row in buf)
    ...
    def _draw_vehicle_sprites(self, buf, player, zbuf, cam, W, H, horizon, view_angle):
        for v in player.vehicles:
            ...
            # Project relative to view_angle (not car angle) for correct look-around
